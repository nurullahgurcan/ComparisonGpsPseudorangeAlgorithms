...
% NURULLAH - it may a fault
index=find(satOrbits(1).TOW==TOW);
% index=find(satOrbits.TOW==TOW);
PRNlist=[];
...
broadcast_obs.col.S1=7;     % NURULLAH
broadcast_obs.col.S2=8;     % NURULLAH
...
for ii=1:length(PRNlist)
    ...
    broadcast_obs.data(ii,broadcast_obs.col.S1)=satOrbits(PRNlist(ii)).S1(index);    % NURULLAH
    broadcast_obs.data(ii,broadcast_obs.col.S2)=satOrbits(PRNlist(ii)).S2(index);    % NURULLAH
end
