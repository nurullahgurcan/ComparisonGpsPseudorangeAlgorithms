function [Pos_KF] = extended_kalman(SV_Pos_case, SV_Rho_case)
...
load SV_Pos_default                         % position of satellites
load SV_Rho_default                         % pseudorange of satellites  
...
X([1 3 5]) = [4.849208586075141e+06
                -3.603361348398131e+05
                    4.114917853453075e+06];                 %Initial position

X([2 4 6]) = [0 0 0];                                            %Initial velocity
% X(7,1) = 3.575261153706439e+006;                                 %Initial clock bias
X(7,1) = 1.602521494466086;
...
% for ii = 1:N
    ...
    g = @(X) PseudorangeEquation(X, (SV_Pos_case)'); 

    % Set R
    Rhoerror = 360;                                               % variance of measurement error(pseudorange error)
    R = eye(size((SV_Pos_case)', 1)) * Rhoerror; 

    % Set Z
    Z = SV_Rho_case.';                                            % measurement value

    [X,P] = Extended_KF(f,g,Q,R,Z,X,P);
    Pos_KF = X([1 3 5]).';                                           % positioning using Kalman Filter
    Pos_LS = Rcv_Pos_Compute((SV_Pos_case)', SV_Rho_case);      % positioning using Least Square as a contrast
