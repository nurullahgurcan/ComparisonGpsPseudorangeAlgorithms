...
for ii=1:length(TimeSpan)
    ...
    [pos_weighted_multilateration(ii,1:4)] = weighted_algebraicGPSequations(satpos, obs_pseudo, weight);
    pos_extended_kalman_filter(ii,:) = extended_kalman(satpos, obs_pseudo);
end

% NURULLAH
plot_err(rec_xyz_actual, epochs, pos_least_squares, pos_sigma_weighted_ls, ...
    pos_exp_weighted_ls, pos_multilateration, pos_weighted_multilateration);